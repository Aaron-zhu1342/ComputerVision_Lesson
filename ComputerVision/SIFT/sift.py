...
if __name__ == '__main__':
    ...
    keyPoints2 = SIFT(img2,True)

    for k in keyPoints:
        cv2.circle(origimg, (int(k[0]), int(k[1])), 2, (255, 255, 0), 1)

    for k in keyPoints2:
        cv2.circle(origimg2,(int(k[0]),int(k[1])),2,(255,255,0),1)

    cv2.imshow('1', origimg)
    cv2.imshow('2',origimg2)
    cv2.waitKey(0)
    cv2.destroyAllWindows()


    #
    # kp1 = cv2.KeyPoint(keyPoints)
    # img3 = cv2.drawKeypoints(origimg, kp1, origimg, color=(255, 0, 255))
    # cv2.imshow('',img3)
    # cv2.waitKey(0)
    # cv2.destroyAllWindows()
